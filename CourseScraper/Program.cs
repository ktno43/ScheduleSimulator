...
namespace CourseScraper
{
    class Program
    {
        private static ChromeDriver driver = null; // Chrome driver
        ...
        {
            if (doc.GetElementbyId(id).InnerText.Equals("&nbsp;")) // If non breaking space return empty
            {
                return "";
            }
            ...
            {
                return System.Net.WebUtility.HtmlDecode(doc.GetElementbyId(id).InnerText); // Return decoded string
            }
        }
        ...
        private static void scrapCourseDb()
        {
            ...
            SelectElement subjectDdList = new SelectElement(driver.FindElementById(Properties.MySettings.Default.SUBJECT_DDL)); // Select subject drop down list
            SelectElement termDdList = new SelectElement(driver.FindElementById(Properties.MySettings.Default.TERM_DDL)); // Select term drop down list

            termDdList.SelectByText(Properties.MySettings.Default.TERM); // Select term
